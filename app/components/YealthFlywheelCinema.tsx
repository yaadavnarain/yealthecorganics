/**
 * YealthFlywheelCinema — VARIANT C "Cinematic depth" of the baseline YealthFlywheel.
 *
 ...
 *  - depth layering strictly inside the composition: a darker, desaturated
 *    back row of distant farm silhouettes at ~20% opacity behind the main farms
 *  - all coin/leaf flight animations use ease-in-out (per-segment) instead of
 *    linear, so motion accelerates/decelerates physically
 *  - a very subtle breathing scale (1.00 -> 1.012) on the whole composition
 *  - NO floating particles or bubbles anywhere
 *
 * Identical 32s master timeline, story beats, label text, and reduced-motion
 * fallback as the baseline. Namespaced yfc*. Pure SVG + CSS, zero runtime JS.
 */

const css = `
  .yfc-wrap{margin-top:8px}
  .yfc{width:100%;height:auto;display:block}

  /* whole-composition breathing */
  .yfc .breathe{
    transform-box:fill-box;transform-origin:center;
    animation:yfcBreathe 8s ease-in-out infinite;
  }

  .yfc .shed{fill:url(#yfcShedGrad);stroke:#F5C842;stroke-width:2}
  .yfc .shelf{stroke:rgba(245,200,66,.30);stroke-width:2}
  .yfc .backrow{opacity:.2}
  .yfc .back-shed{fill:#161616;stroke:#3A3A3A;stroke-width:2}
  .yfc .caption{
  ...
  .yfc .guide{
    ...
    animation:yfcDash 3s linear infinite;
  }

  .yfc .farm-inner{
    transform-box:fill-box;transform-origin:center bottom;opacity:0;
  }
  .yfc .f1i{animation:yfcFarm1 32s linear infinite}
  .yfc .f2i{animation:yfcFarm2 32s linear infinite}
  .yfc .f3i{animation:yfcFarm3 32s linear infinite}

  .yfc .plant rect{
    fill:#34D399;
    transform-box:fill-box;transform-origin:center bottom;
    animation:yfcGrow 8s linear infinite;
  }

  /* flights: ease-in-out between keyframes for physical motion */
  .yfc .coin-out{opacity:0;offset-rotate:0deg}
  .yfc .co1{animation:yfcCoinOut1 32s ease-in-out infinite}
  .yfc .co2{animation:yfcCoinOut2 32s ease-in-out infinite}
  .yfc .co3{animation:yfcCoinOut3 32s ease-in-out infinite}

  .yfc .ret{opacity:0;offset-rotate:0deg}
  .yfc .ret1{animation:yfcRet1 32s ease-in-out infinite}
  .yfc .ret2{animation:yfcRet2 32s ease-in-out infinite}
  .yfc .ret3{animation:yfcRet3 32s ease-in-out infinite}
  .yfc .retleaf{animation:yfcLeafFade 8s linear infinite}
  .yfc .retcoin{opacity:0;animation:yfcCoinFade 8s linear infinite}

  .yfc .wallet-inner{
    transform-box:fill-box;transform-origin:center;
    animation:yfcPulse 32s ease-in-out infinite;
  }

  .yfc .stack ellipse{fill:url(#yfcStackGrad);stroke:#0A0A0A;stroke-width:1;opacity:0;transform-box:fill-box}
  .yfc .s1{animation:yfcStack1 32s linear infinite}
  .yfc .s2{animation:yfcStack2 32s linear infinite}
  .yfc .s3{animation:yfcStack3 32s linear infinite}
  .yfc .s4{animation:yfcStack4 32s linear infinite}
  .yfc .s5{animation:yfcStack5 32s linear infinite}
  .yfc .s6{animation:yfcStack6 32s linear infinite}
  ...
  .yfc .lbl .g{fill:#F5C842}
  .yfc .lbl1{animation:yfcLbl1 32s linear infinite}
  .yfc .lbl2{animation:yfcLbl2 32s linear infinite}
  .yfc .lbl3{animation:yfcLbl3 32s linear infinite}
  .yfc .lbl4{animation:yfcLbl4 32s linear infinite}
  .yfc .lbl5{animation:yfcLbl5 32s linear infinite}
  .yfc .lbl6{animation:yfcLbl6 32s linear infinite}
  .yfc .lbl7{animation:yfcLbl7 32s linear infinite}
  .yfc .lbl8{animation:yfcLbl8 32s linear infinite}
  .yfc .lbl9{animation:yfcLbl9 32s linear infinite}
  .yfc .lbl10{animation:yfcLbl10 32s linear infinite}
  .yfc .lbl11{animation:yfcLbl11 32s linear infinite}
  .yfc .lbl12{animation:yfcLbl12 32s linear infinite}
  .yfc .lbl-static{opacity:0}

  @keyframes yfcBreathe{
    0%,100%{transform:scale(1)}
    50%{transform:scale(1.012)}
  }

  @keyframes yfcDash{to{stroke-dashoffset:-60}}
  ...
  @keyframes yfcLeafFade{0%,52%{opacity:1}58%,100%{opacity:0}}
  @keyframes yfcCoinFade{0%,52%{opacity:0}58%,100%{opacity:1}}
  ...
  @media (prefers-reduced-motion: reduce){
    .yfc *{animation:none !important}
    .yfc .breathe{animation:none !important;transform:none !important}
    .yfc .farm-inner{opacity:1 !important;transform:none !important}
    .yfc .plant rect{transform:none !important;opacity:1 !important}
    .yfc .stack ellipse{opacity:1 !important;transform:none !important}
    .yfc .ret,.yfc .coin-out,.yfc .lbl{opacity:0 !important}
    .yfc .lbl-static{opacity:1 !important}
    .yfc .guide{animation:none !important}
  }
`;
...
function FarmInner({ cls, label }: { cls: string; label: string }) {
  return (
    <g className={`farm-inner ${cls}`}>
      <ellipse cx="0" cy="5" rx="50" ry="7" fill="url(#yfcShadow)" />
...
export default function YealthFlywheelCinema() {
  return (
    <div className="yfc-wrap" style={{ width: "100%", maxWidth: 760, margin: "0 auto" }}>
      <style dangerouslySetInnerHTML={{ __html: css }} />
      <svg className="yfc" viewBox="0 0 800 460" role="img"
        ...
        <defs>
          <radialGradient id="yfcCoinGrad" cx="35%" cy="30%" r="80%">
            <stop offset="0%" stopColor="#FFE9A0" />
            <stop offset="40%" stopColor="#F5C842" />
            <stop offset="85%" stopColor="#D9A521" />
            <stop offset="100%" stopColor="#C2941C" />
          </radialGradient>
          <radialGradient id="yfcStackGrad" cx="40%" cy="35%" r="80%">
            <stop offset="0%" stopColor="#FFE08A" />
            <stop offset="55%" stopColor="#F5C842" />
            <stop offset="100%" stopColor="#D9A521" />
          </radialGradient>
          <linearGradient id="yfcShedGrad" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#222222" />
            <stop offset="100%" stopColor="#0F0F0F" />
          </linearGradient>
          <linearGradient id="yfcWalletGrad" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#242424" />
            <stop offset="100%" stopColor="#0F0F0F" />
          </linearGradient>
          <radialGradient id="yfcShadow" cx="50%" cy="50%" r="50%">
          ...
          <g id="yfcCoin">
            ...
            <circle r="12" fill="url(#yfcCoinGrad)" />
          ...
          {/* distant farm silhouette for the back row */}
          <path id="yfcBackFarm" className="back-shed" d="M -40 0 L -40 -75 A 40 40 0 0 1 40 -75 L 40 0 Z" />
        </defs>

        <g className="breathe">
          {/* depth layer: darker desaturated farm silhouettes behind the main row */}
          <g className="backrow" aria-hidden="true">
            <use href="#yfcBackFarm" transform="translate(500,292) scale(0.55)" />
            <use href="#yfcBackFarm" transform="translate(608,288) scale(0.48)" />
            <use href="#yfcBackFarm" transform="translate(706,294) scale(0.58)" />
          </g>
          ...
          {/* wallet (you) */}
          <ellipse cx="130" cy="264" rx="52" ry="8" fill="url(#yfcShadow)" />
          <g transform="translate(130,230)">
            <g className="wallet-inner">
              <rect x="-42" y="-28" width="84" height="56" rx="10" fill="url(#yfcWalletGrad)" stroke="#F5C842" strokeWidth="2" />
          ...
          <ellipse cx="130" cy="316" rx="42" ry="6" fill="url(#yfcShadow)" opacity=".8" />
          ...
          {/* outbound coins (eased flights) */}
          <g className="coin-out co1" style={{ offsetPath: "path('M 178 218 C 300 90 470 100 560 200')" }}><use href="#yfcCoin" /></g>
          <g className="coin-out co2" style={{ offsetPath: "path('M 178 218 C 320 70 560 80 665 200')" }}><use href="#yfcCoin" /></g>
          <g className="coin-out co3" style={{ offsetPath: "path('M 178 218 C 280 110 400 110 455 200')" }}><use href="#yfcCoin" /></g>

          {/* returns (eased flights) */}
          <g className="ret ret1" style={{ offsetPath: "path('M 560 200 C 480 360 300 370 178 240')" }}>
            <use className="retleaf" href="#yfcLeaf" />
            <use className="retcoin" href="#yfcCoin" />
          </g>
