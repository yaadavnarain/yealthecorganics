/**
 * YealthFlywheelD — VARIANT D "3D greenhouse & wallet" of the baseline YealthFlywheel.
 *
 * Structural base: Variant B (iso light direction, coin flight scaling with
 * synced travelling shadows, gradient coins/stack, ground shadows). Artwork
 * inside the animated groups is fully redesigned:
 *
 *  FARMS (after refs/greenhouse-ref.png + refs/aquaponics-ref.jpg):
 *   - small gabled glasshouses with a front gable face, one receding right
 *     side wall and roof plane (consistent upper-left light), chunky dark
 *     frame with thin gold edge accents
 *   - mint-tinted translucent glass panes with mullions; visible through the
 *     front glass: two raft rows of 3D lettuce heads (overlapping mint
 *     circles with darker base + lighter crown) on pale boards over a dark
 *     water strip
 *   - the lettuce heads carry the original 8s grow/harvest animation
 *
 *  WALLET (after refs/wallet-ref.png, brand colors — wallet only, no arrow
 *  or dollar coin): rounded charcoal bifold with visible fold depth, gold
 *  strap band with clasp plate + circular button, soft top highlight.
 *
 * Identical 32s master timeline, story beats, labels, and reduced-motion
 * fallback. Namespaced yfd*. Pure SVG + CSS, zero runtime JS, no blur filters.
 */
...
/**
 * Gabled glasshouse after refs/greenhouse-ref.png: front gable face plus a
 * receding right side wall and roof plane (light from upper-left), chunky
 * dark frame with gold edge accents, translucent mint glass, and an
 * aquaponics interior (raft rows of lettuce over a dark water strip).
 */
...
export default function YealthFlywheelD() {
  return (
    <div className="yfd-wrap" style={{ width: "100%", maxWidth: 760, margin: "0 auto" }}>
      ...
      <svg className="yfd" viewBox="0 0 800 460" role="img"
        ...
        {/* income stack (unchanged from base) */}
